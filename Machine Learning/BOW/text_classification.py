...
def training(tr_x, tr_y, Vec):
    ...
    # save the model
    joblib.dump(model, 'models/model_SVC_2gram.pkl')
    joblib.dump(Vec, 'models/vec_SVC_2gram.pkl')
    return model
...
def main():
    ...
    f.close()
    # print(stop_words[50])
    Vec = TfidfVectorizer(stop_words=stop_words, tokenizer=jieba_tokenizer, lowercase=False, ngram_range=(1, 2))
    model = training(tr_x, tr_y, Vec)
    # Vec = joblib.load('models/vec_SVC.pkl')
    # model = joblib.load('./models/model_SVC.pkl')
    print(1)
    testing(model, Vec)
    # res = "A股大涨"
    # print(model.predict(Vec.transform([res])))
    tr.close()
